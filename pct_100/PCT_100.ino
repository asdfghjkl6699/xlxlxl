...
void setup() {
  ...
  // 开机万色灯闪烁提示（持续2秒）
  Serial.println("开机万色灯闪烁...");
  rgb_led_rainbow(2000);
...
void loop() {
  ...
  // ===================== 【RGB LED状态指示】 =====================
  // 确定当前指示灯状态（优先级从高到低）
  // WiFi连接状态：只有WL_CONNECTED表示已连接，其他状态都视为未连接
  ...
  // 总开关关闭时，关闭万色灯
  if (!key1_is_on()) {
      rgb_led_set_rgb(0, 0, 0);  // 关闭LED
  } else {
      if (led_on && fan_on) {
          // 严重警报：LED和风扇都启动
          rgb_led_set_status(RGB_STATUS_SEVERE);
      } else if (led_on) {
          // 光照越界：LED灯亮
          rgb_led_set_status(RGB_STATUS_ALARM_LIGHT);
      } else if (fan_on) {
          // 温度越界：风扇启动
          rgb_led_set_status(RGB_STATUS_ALARM_TEMP);
      } else if (!mqtt_connected && wifi_connected) {
          // MQTT未连接（但WiFi已连接）
          rgb_led_set_status(RGB_STATUS_MQTT_DISCONNECTED);
      } else if (!wifi_connected) {
          // WiFi未连接
          rgb_led_set_status(RGB_STATUS_WIFI_DISCONNECTED);
      } else {
          // 所有状态正常
          rgb_led_set_status(RGB_STATUS_NORMAL);
      }
      rgb_led_update();  // 只有总开关打开时才更新LED状态
  }
  ...
  if (Serial.available() > 0) {
    ...
    // 设置设备ID命令: ID PCT_100_005
    if (cmd.startsWith("ID ")) {
      String new_id = cmd.substring(3);
      new_id.trim();
      if (new_id.length() > 0) {
        mqtt_mgr_set_device_id(new_id.c_str());
        Serial.printf("设备ID已设置为: %s\n", new_id.c_str());
      }
    }
    // 获取设备ID命令: ID?
    else if (cmd == "ID?") {
      Serial.printf("当前设备ID: %s\n", mqtt_mgr_get_device_id());
    }
    // 断开WiFi连接
    ...
    // WiFi状态查询
      ...
      Serial.print("MQTT状态: ");
      Serial.println(mqtt_mgr_is_connected() ? "已连接" : "已断开");
    }
    // 断开MQTT连接
    else if (cmd == "mqtt_disconnect") {
      mqtt_mgr_disconnect();
    }
    // 重新连接MQTT
    else if (cmd == "mqtt_reconnect") {
      mqtt_mgr_reconnect_now();
    }
    // 显示帮助命令
    else if (cmd == "HELP") {
      Serial.println("=== 串口命令列表 ===");
      Serial.println("ID <new_id>   - 设置设备ID");
      Serial.println("ID?           - 获取当前设备ID");
      Serial.println("disconnect    - 断开WiFi连接");
      Serial.println("clear         - 清除WiFi配置");
      Serial.println("reconnect     - 重新连接WiFi");
      Serial.println("wifi          - WiFi配置向导");
      Serial.println("mqtt_disconnect - 断开MQTT连接");
      Serial.println("mqtt_reconnect  - 重新连接MQTT");
      Serial.println("status        - 查询WiFi状态");
  ...
  if (!key1_is_on()) {
    ...
    bool wifi_connected = (WiFi.status() == WL_CONNECTED);
    bool mqtt_connected = mqtt_mgr_is_connected();
    lcd_update(is_auto_mode, false, light_intensity, light_thresh_display, current_temp, g_temp_threshold, false, false, wifi_connected, mqtt_connected);
    return;
  }
  ...
  if (last_key2_state && !key2_state && key2_holding) {
    ...
    if (hold_time < LONG_PRESS_MIN && !key2_processed) {
      if (!is_auto_mode) {
        step++;
        switch (step) {
          case 1: current_state = S00; break;
          case 2: current_state = S01; break;
          case 3: current_state = S10; break;
          case 4: current_state = S11; step = 0; break;
        }
        setRelay(current_state);
